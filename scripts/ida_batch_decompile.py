        ...
        for f in IdaHelper.get_functions():
            try:
                ...
                if '**** XREFS ****' in function_comment:
                    logger.debug(
                        "[i] skipping function %r, already annotated." % f.name)
                    continue
...
class IdaDecompileBatchController(object):
    def __init__(self):
        ...
        self.is_ida64 = GetIdbPath().endswith(".i64")
    ...
    def run(self):
        ...
        if self.chk_decompile_imports:
            ...
            for image_type, image_name, image_path in self.enumerate_import_images():
                try:
                    ...
                    logger.warning(
                        "[!] failed to decompile %r - %r" % (image_path, cpe))
        ...
        logger.info("    output dir: %s" %
                    self.output_path if self.output_path else self.target_dir)
    ...
    def decompile_all(self, outfile=None):
        outfile = self._get_suggested_output_filename(
            outfile or self.target_path)
    ...
    def _exec_ida_batch(self, target, command):
        # build exe path
        if self.is_windows:
            ida_exe = os.path.join(
                self.ida_home, 'idaw64.exe' if self.is_ida64 else 'idaw.exe')
        else:
            ida_exe = os.path.join(
                self.ida_home, 'idal64' if self.is_ida64 else 'idal')
        '''
...
class DecompileBatchForm(Form):
    ...
    def __init__(self, idbctrl, enumerate_imports=True, enumerate_other=False):
        ...
        self.EChooser = TestEmbeddedChooserClass(
            "Batch Decompile", flags=Choose2.CH_MULTI)
        self.propagateItems(enumerate_imports=enumerate_imports,
                            enumerate_other=enumerate_other)
        Form.__init__(self,
...
""", {
    ...
    def OnButtonProcess(self, code=0):
        ...
        if not len(list(self.EChooser.getSelected())):
            logger.warning(
                "[!] Aborting. Please select at least one item from the list!")
            return
        ...
        for _type, name, image_path in self.EChooser.getSelected():
            ...
            try:
                ...
                logger.warning("[!] failed to decompile %r - %r" %
                               (image_path, cpe))
        ...
        if decompile_main_binary:
            ...
            logger.info("[+] finished decompiling: %r" % files_decompiled)
            logger.info("    output dir: %s" %
                        self.idbctrl.output_path if self.idbctrl.output_path else self.idbctrl.target_dir)
...
class IdaDecompileBatchPlugin(idaapi.plugin_t):
    ...
    def init(self):
        ...
        if idaapi.IDA_SDK_VERSION >= 700:
            ...
            action_desc = idaapi.action_desc_t(
                "tintinweb:batchdecompile:load", self.wanted_name, IdaDecompileUiActionHandler(self))
    ...
    def set_ctrl(self, idbctrl):
        logger.debug("[+] %s.set_ctrl(%r)" %
                     (self.__class__.__name__, idbctrl))
        self.idbctrl = idbctrl
